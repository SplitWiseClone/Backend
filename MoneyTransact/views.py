...
class FilterTransactionsOnCategoryView(APIView):
    """
    List all transactions, or create a new transaction.
    """
    permission_classes = (permissions.IsAuthenticated,)
    ...
    def get(self, request, category, format=None):
        my_transactions = (Transaction.objects.filter(taker=str(request.user.id), active=True, category=category)
                           | Transaction.objects.filter(giver=str(request.user.id), active=True, category=category)).distinct()
        transactions_details = list()
        for transaction in my_transactions:
            print("transaction ", transaction)
            takers_set = set()
...
class UpdateTransactionView(APIView):
    ...
    def patch(self, request, transaction_id, format=None):
        transaction = Transaction.objects.get(id=transaction_id)
        print("transaction ", transaction)
